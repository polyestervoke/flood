 ...
 * @summary Extracts WebVTT subtitle from a file of a torrent.
 * @tags Torrent
 * @security User
 * @param {string} hash.path
 * @param {string} index.path - index of selected file
    ...
    req.services?.clientGatewayService
      ...
      .then(async (contents) => {
        ...
        if (!fileExt || !subtitleExtensions.includes(fileExt)) {
          res.status(422).json({message: 'Cannot extract subtitles for this file.'});
          return;
        }
